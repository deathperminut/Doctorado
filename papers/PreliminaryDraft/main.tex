...
To further constrain the solution manifold and exploit learned representations from the forward model, a cycle-consistent framework is adopted. A complementary inverse model is trained to map generated images back into their corresponding Hamiltonian parameters. The cycle consists of:

\begin{enumerate}
    \item A forward generator $G_\theta$: $(\mathbf{p}) \rightarrow (\hat{X})$ mapping parameters to images.
    \item An inverse encoder $F_\phi$: $(\hat{X}) \rightarrow (\hat{\mathbf{p}})$ recovering the parameter vector.
\end{enumerate}

Cycle consistency is enforced through a constraint of the form:
\begin{equation}
\mathcal{L}_{\mathrm{cycle}} = \left\| F_\phi(G_\theta(\mathbf{p})) - \mathbf{p} \right\|_2^2,
\end{equation}
ensuring that reconstructed parameters remain consistent with the physical conditions initially used to generate the synthetic image. This mechanism reduces ambiguity and improves identifiability under degeneracy, leveraging the shared latent structure between forward and inverse models.

\subsection{Bayesian Hyperparameter Optimization}

To maximize model robustness and ensure reliable convergence under multimodal image distributions, Bayesian optimization is used to tune key hyperparameters such as $\beta$, $\gamma$, latent dimensionality, learning rates, and conditioning network depth. This optimization strategy provides a principled approach to exploring the hyperparameter space under uncertainty, improving performance while minimizing empirical trial-and-error.

\subsection{Methodology Summary}

The methodology combines:
\begin{itemize}
    \item physics-based synthetic data generation via atomistic Hamiltonian simulations,
    \item a conditional VAE model informed by Hamiltonian priors,
    \item explicit physical regularization through Hamiltonian-derived energy terms,
    \item cycle consistency between forward and inverse mappings, and
    \item Bayesian optimization for selecting optimal model configurations.
\end{itemize}

This integrated framework establishes a physically grounded approach for mitigating magnetic state degeneracy in nanoscale magnetic imaging tasks.
