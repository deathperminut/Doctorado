...
\paragraph{Evaluation protocols and interpretability metrics.}
A persistent deficiency is the lack of standardized interpretability metrics tailored to physics. Common image-similarity metrics (SSIM, PSNR) do not capture physical fidelity. Promising evaluation practices include per-term Hamiltonian residuals, domain-feature localization error (e.g., domain-wall shift), calibrated parameter-recovery metrics with coverage probabilities, and counterfactual consistency tests (modify a parameter and verify that the predicted change in texture matches the simulator). Combining qualitative attribution with these physics-aware quantitative metrics increases confidence in interpretability claims \cite{Oviedo2022, Li2023}.
