...
\newacronym{spstm}{SP-STM}{Spin-Polarized Scanning Tunneling Microscopy}
\newacronym{mram}{MRAM}{Magnetic Random-Access Memory}
\newacronym{dmi}{DMI}{Dzyaloshinskii–Moriya Interaction}
\newacronym{llg}{LLG}{Landau–Lifshitz–Gilbert}
\newacronym{mri}{MRI}{Magnetic Resonance Imaging}
\newacronym{mnp}{MNP}{Magnetic Nanoparticle}
\newacronym{mnps}{MNPs}{Magnetic Nanoparticles}
\newacronym{tem}{TEM}{Transmission Electron Microscopy}
\newacronym{stt}{STT}{Spin-Transfer Torque}
\newacronym{sot}{SOT}{Spin-Orbit Torque}
\newacronym{cvae}{cVAE}{Conditional Variational Autoencoder}
\newacronym{gan}{GAN}{Generative Adversarial Network}
\newacronym{cnn}{CNN}{Convolutional Neural Network}
\newacronym{ram}{RAM}{Regression Activation Map}
\newacronym{eeg}{EEG}{Electroencephalographic}
...
probe resolution, tip–sample convolution in \gls{mfm}, and sample variability—hinder quantitative
interpretation of \gls{mfm} or \gls{spstm} images \cite{Makarova2022, Bagchi2024}.

Deep learning has emerged as a powerful tool to bridge this gap. The inverse problem—inferring
physical parameters from magnetic domain images—has been successfully addressed using \glspl{cnn} and
...
generative models such as VAEs and conditional \glspl{gan} \cite{MicroVAE2023, Ronne2024}. However, three fundamental challenges remain unresolved. First, \textit{parameter degeneracy}—where distinct Hamiltonian parameter combinations produce visually similar magnetic textures—complicates unique identification and prevents robust inverse estimation \cite{Feng2024, Liu2023, DeepSPIN2023}. Second, \textit{data quality degradation} arising from experimental noise, finite probe resolution, and numerical discretization in simulations limits the reliability of both training data and model predictions \cite{Kramer2021MFMNoise, Exl2021NumericalErrors, Park2024}. Third, the \textit{lack of physics-based interpretability} in data-driven models obscures the connection between learned representations and fundamental magnetic interactions, hindering validation against established physical laws and limiting their utility for materials design \cite{Rao2022ExplainableMaterialsML, Chen2021PINNMagnetism, Sanchez2023ExplainableDesign}. Existing generative approaches can reproduce general morphologies but suffer from poor interpretability, instability under noise, limited generalization, and lack of physical constraints \cite{Feng2024, Park2024, MicroVAE2023}.
...
This work proposes a unified deep learning framework to address three fundamental challenges in nanoscale magnetic domain characterization: parameter degeneracy, data quality degradation, and lack of interpretability. \textbf{First}, to mitigate the effects of magnetic state degeneracy, the framework integrates a physically informed \gls{cvae} with cycle-consistent forward--inverse modeling. The \gls{cvae} employs parameter-dependent latent priors and physics-based energy penalties—including exchange, \gls{dmi}, anisotropy, and Zeeman contributions—to enforce physically plausible domain generation. A complementary inverse estimator based on transfer learning performs Hamiltonian parameter regression from spin configurations. Both models are jointly optimized under a cycle-consistency constraint to ensure round-trip parameter identifiability. \textbf{Second}, to enhance the quality of magnetic states under degraded data conditions, diffusion-based generative strategies and domain adaptation techniques will be explored to bridge high-fidelity simulations and noisy experimental \gls{mfm}/\gls{spstm} observations, incorporating denoising and adversarial alignment methods while preserving physical structure. \textbf{Third}, to provide physics-based interpretability, the framework employs \glspl{ram} to identify spatially relevant features contributing to parameter predictions, combined with UMAP analysis of latent representations to verify alignment with physical regimes \cite{Sparks2024, Optuna2022}.

This research addresses critical technological needs at the national and regional level, particularly in advancing Colombia's capabilities in emerging materials characterization and intelligent data processing. The proposed framework directly supports the development of next-generation magnetic memory devices and energy storage systems—technologies identified as strategic priorities for national innovation and competitiveness. By integrating physics-informed deep learning with experimental microscopy workflows, this work contributes to bridging the gap between fundamental materials science and industrial applications in spintronics and magnetic-based energy technologies. Furthermore, this research aligns with the mission of the Digital Signal and Control Processing Group (GCPDS), which focuses on developing advanced computational methods for signal and image analysis. The integration of multimodal explainability techniques, physics-based regularization, and robust inference under noisy data conditions extends the group's expertise in digital image processing into the domain of nanoscale materials characterization, fostering interdisciplinary collaboration between materials physics, machine learning, and computational imaging. This convergence positions Colombia to contribute to global efforts in accelerating materials discovery through data-driven approaches while building local capacity in high-impact research areas with direct technological and economic implications.
...
modern materials science \cite{Wang2022, Lak2021}. In particular, \glspl{mnp} and nanodots
...
scales, the balance between exchange interactions, anisotropy, \gls{dmi}, and external magnetic fields generates a wide variety of magnetic
textures—vortices, skyrmions, helical domains, and labyrinthine configurations, as illustrated in Figure~\ref{fig:magnetic_domains} \cite{Ga2022, Fert2023, Tokura2021}. These
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Figures/MagneticDomainsConfigurations.pdf}
    \caption{Representative magnetic domain configurations in nanoscale systems: (a) vortex states with in-plane curling magnetization, (b) skyrmion configurations stabilized by Dzyaloshinskii–Moriya interaction, (c) helical and stripe domain patterns, and (d) labyrinthine domains from dipolar interactions.}
    \label{fig:magnetic_domains}
\end{figure}

%1.2. Relevancia aplicada: por qué las nanopartículas magnéticas importan

Magnetic NPs and nanodots possess a unique versatility that makes them relevant across multiple technological and scientific domains (Figure~\ref{fig:applications}). In the field of spintronics and non-volatile memory technologies, topologically protected magnetic textures—such as skyrmions and magnetic vortices—enable the encoding and manipulation of information as stable, switchable magnetic bits. These nanoscale spin configurations support the development of next-generation memory architectures including \gls{mram}, racetrack memory devices, and reconfigurable magnetic logic gates. The appeal of these technologies lies in their combination of high storage density, ultralow power consumption, radiation hardness, and non-volatility, making them promising candidates for replacing conventional charge-based electronics in applications ranging from edge computing to space-grade systems \cite{Morshed2021, Sisodia2022, Zhang2015, Eurek2024}. Beyond information processing, the dynamic magnetic response of these nanostructures under applied fields or current-induced torques allows for fast and deterministic switching, essential for achieving competitive read/write speeds in practical memory devices.

In parallel, magnetic NPs have emerged as transformative agents in biomedical and energy-related applications, where their spin dynamics and domain structure directly govern functional performance. In biomedicine, magnetic NPs are employed in targeted oncological hyperthermia therapies, where alternating magnetic fields induce localized heating to selectively destroy tumor cells, as well as in advanced drug delivery systems that exploit magnetic guidance for site-specific therapeutic action \cite{Barrera2023, IronOxideReview2023, MLDesign2023}. Their utility extends to contrast enhancement in \gls{mri}, where tailored magnetic anisotropy and relaxation properties improve diagnostic sensitivity. In the energy sector, magnetic NPs are investigated for magnetocaloric refrigeration—a sustainable alternative to vapor-compression cooling—thermoelectric energy conversion, and reversible magnetic energy storage systems \cite{He2025, Nehan2024, Hadouch2022}. In all these contexts, the efficiency of the functional response is fundamentally determined by the internal magnetic domain structure, anisotropy landscape, and the mechanisms of energy transfer between spin, lattice, and thermal degrees of freedom \cite{Liedienov2021, Nehan2024}. This broad spectrum of applications underscores the critical need for robust characterization and predictive modeling frameworks capable of linking Hamiltonian parameters to experimentally observable magnetic configurations.
...
Theoretical and computational approaches based on Hamiltonian formulations have become the dominant framework for studying magnetic nanodots due to their ability to provide physically grounded predictions under controlled parameter conditions. Micromagnetic simulations and atomistic spin dynamics solve the \gls{llg} equation or minimize the magnetic Hamiltonian to predict equilibrium and dynamic states \cite{Donahue1999OOMMF, MuMaxReview2021, Evans2014Atomistic}. This approach offers several critical advantages: it enables exploration of parameter regimes inaccessible to experiment, provides complete three-dimensional vector field information at arbitrary resolution, and allows systematic investigation of competing magnetic interactions—exchange, anisotropy, \gls{dmi}, Zeeman coupling—without measurement-induced perturbations \cite{Hertel2022MicromagneticLimits, Hinzke2022Atomistic}. Moreover, simulations can incorporate thermal fluctuations, boundary conditions, and time-dependent external fields with explicit physical control, making them indispensable for understanding emergent phenomena such as skyrmion nucleation, domain wall dynamics, and magnetization reversal pathways \cite{Hertel2023Thermal, Brown1963Thermal}.

However, despite their physical rigor, Hamiltonian-based simulations reveal a fundamental challenge: the degeneracy of magnetic states. Different combinations of parameters—such as exchange interaction, anisotropy, and \gls{dmi}—can produce visually similar or energetically comparable magnetic patterns \cite{Feng2024, Liao2023, DeepSPIN2023}. As illustrated in Figure~\ref{fig:degeneracy_problem}, spin configurations sampled across the KDM (a proxy for \gls{dmi} strength) and temperature parameter space exhibit striking visual similarity across distinct parameter regimes. This degeneracy arises because the magnetic energy landscape contains multiple local minima corresponding to metastable spin textures—vortices, skyrmions, stripe domains—that can be stabilized under different parameter sets \cite{Leonov2020EnergyLandscape, Du2022MetaStableSkyrmions}. Small compensations between competing interactions can preserve the overall energy balance: for instance, an increase in anisotropy may be offset by stronger exchange or weaker \gls{dmi}, yielding morphologically indistinguishable configurations \cite{Liu2023InverseML, Bessarab2018EnergyBarriers}. The visual similarity of domain textures across this parameter space creates fundamental ambiguity in inverse estimation tasks, complicating the unique identification of physical parameters from observed configurations and limiting the predictive power of both simulation-based and data-driven workflows \cite{Park2024, Raju2022SkyrmionInverse}.
...
phenomena \cite{Rao2022ExplainableMaterialsML, Park2024, DeepSPIN2023}. As illustrated in Figure~\ref{fig:interpretability_challenge}, this opacity complicates the validation of learned relationships against es-
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Figures/monte_carlo_granular_flow.pdf}
...
In parallel, physics-informed neural networks (PINNs) and hybrid modeling frameworks have gained traction by embedding the Landau–Lifshitz–Gilbert (\gls{llg}) equation or energetic constraints directly into the learning objective \cite{Karniadakis2021PINNs, Chen2021PINNMagnetism}. As illustrated in Figure~\ref{fig:pinns_flow}, PINNs augment the standard neural surrogate architecture with physics-based loss terms that enforce the \gls{llg} equation at collocation points throughout the spatiotemporal domain. These approaches have been tested in applications such as reconstructing domain-wall dynamics, modeling low-dimensional magnetic textures, or enforcing physical plausibility in neural surrogate outputs. By injecting physical laws into the learning process, they help reduce the feasible set of solutions and thereby restrict degeneracy. However, PINNs for magnetic systems remain difficult to train due to the stiffness of the \gls{llg} equation, challenges in balancing data fidelity and physics terms, and poor scalability to realistic thin-film geometries and finite-temperature regimes. Their use in nanoscale magnetic domain reconstruction remains exploratory rather than widely adopted.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Figures/pinns_flow.pdf}
...
Figure~\ref{fig:simulation_evolution} provides a comprehensive view of the evolution of forward simulation approaches for modeling magnetic state degeneracy, from traditional high-fidelity methods to modern data-driven frameworks.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Figures/simulation_evolution.pdf}
...
trained on large synthetic datasets \cite{Kwon2020, Feng2024}. As illustrated in Figure~\ref{fig:supervised_regression}, these models are
...
PRMaterials2025_phaseboundaries}. Figure~\ref{fig:bayesian_inference} illustrates the Bayesian framework for parameter estimation. By explicitly modeling uncertainty and
...
constraint \cite{DeepSPIN2023, Sanchez2024CycleConsistentMaterials}, as shown in Figure~\ref{fig:cycle_consistent}. By requiring
...
\begin{figure}[H]
    ...
    \caption{Cycle-consistent framework coupling forward and inverse models. Parameters inferred from an image are fed back through the forward model to regenerate the image, enforcing bidirectional consistency and filtering non-physical solutions.}
    \label{fig:cycle_consistent}
\end{figure}
...
As described in the problem statement (Section~\ref{sec:sim2real}), the sim-to-real gap represents a fundamental challenge in applying data-driven models trained on simulated magnetic domain data to experimental observations \cite{Liao2023, Park2024, Hilgers2025}. The systematic distribution shifts between computational predictions and experimental measurements—arising from idealized physics in simulations versus instrumental artifacts, noise, and measurement-specific transformations in experiments—cause learned models to fail when deployed in real-world scenarios \cite{Feng2024, DeepSPIN2023}. This section reviews the state of the art in domain adaptation techniques that have been developed to bridge the simulation-experiment divide, with particular emphasis on methods applicable to magnetic domain imaging and related materials characterization problems. The literature on domain adaptation has matured significantly over the past decade, encompassing data-level transformations, feature-level alignment, model-level transfer, and hybrid approaches that combine multiple strategies \cite{Wilson2020Survey, Wang2018DeepVisualDA}. While originally developed for computer vision tasks (object recognition, semantic segmentation), these methods have shown promise in scientific imaging domains where synthetic training data is abundant but real experimental data is scarce and expensive \cite{Sim2RealReview2021}.

\paragraph{Domain adaptation: overview and categorization.}
Bridging the simulation–experiment gap is commonly addressed through domain adaptation and transfer learning. Domain adaptation techniques enable models trained on source domains (typically simulated data) to generalize effectively to target domains (experimental observations) despite systematic distribution shifts \cite{Ganin2016DomainAdversarial, Sun2016ReturnFrustratingly, Wilson2020Survey}. These methods can be broadly categorized into several complementary approaches, each addressing different aspects of the domain shift problem.

\subparagraph{Data-Based Domain Adaptation.}
As illustrated in Figure~\ref{fig:data_based_da}, data-based approaches modify the training data itself to better match the target domain distribution \cite{Shorten2019DataAugmentation, Perez2017DataAugmentation}. These techniques include classical data augmentation (rotation, scaling, noise injection), synthetic degradation that mimics experimental artifacts (blur, resolution reduction, contrast distortion), and domain mixing strategies that interpolate between source and target samples \cite{Yun2019CutMix, Zhang2018MixUp}. While computationally efficient and requiring no architectural changes, data-based methods depend critically on accurate modeling of the real data distribution and may require extensive manual tuning to capture domain-specific transformations \cite{Cubuk2019AutoAugment, Lim2019FastAutoAugment}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/data_based_domain_adaptation.pdf}
    \caption{Data-based domain adaptation strategies. Synthetic data is transformed through augmentation, degradation, and mixing techniques to reduce the distribution gap with experimental observations.}
    \label{fig:data_based_da}
\end{figure}

\subparagraph{Instance-Based Domain Adaptation.}
Instance-based methods adapt at the sample level by reweighting training instances according to their importance for the target domain or by generating pseudo-labels for unlabeled target data through self-training \cite{Shimodaira2000IW, Grandvalet2005SemiSupervised}. As shown in Figure~\ref{fig:instance_based_da}, these approaches often incorporate test-time adaptation mechanisms that fine-tune model parameters on-the-fly using target domain statistics \cite{Wang2021TentTTA, Sun2020TestTime}. Instance-based adaptation is flexible and requires no target labels, but depends on reliable confidence calibration and can be sensitive to large distribution shifts \cite{Garg2022UnifiedTTA, Niu2022Efficient}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/instance_based_domain_adaptation.pdf}
    \caption{Instance-based domain adaptation approaches. Methods include instance weighting, self-training with pseudo-labels, and test-time adaptation for transferring knowledge across domains.}
    \label{fig:instance_based_da}
\end{figure}

\subparagraph{Feature-Based Domain Adaptation.}
Feature-based methods align the representations learned by neural networks across domains by minimizing statistical discrepancies in intermediate feature spaces \cite{Long2015LearningTransferable, Sun2016DeepCORAL}. As illustrated in Figure~\ref{fig:feature_based_da}, common alignment strategies include Maximum Mean Discrepancy (MMD), which matches feature moments across domains \cite{Long2015LearningTransferable}, CORAL alignment that aligns second-order statistics \cite{Sun2016DeepCORAL}, and Domain-Adversarial Neural Networks (DANN) that learn domain-invariant features through adversarial training \cite{Ganin2016DomainAdversarial}. These approaches are theoretically grounded in domain adaptation theory but require careful hyperparameter tuning and may inadvertently discard task-relevant discriminative features during alignment \cite{Zhao2019LearningTransferable, Ben-David2010Theory}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/feature_based_domain_adaptation.pdf}
    \caption{Feature-based domain adaptation techniques. Shared encoders and statistical alignment methods are used to create domain-invariant feature representations for cross-domain task prediction.}
    \label{fig:feature_based_da}
\end{figure}

\subparagraph{Adversarial Domain Adaptation.}
Adversarial approaches explicitly train domain discriminators alongside task predictors, leveraging gradient reversal layers to encourage feature extractors to produce representations that confound domain classification \cite{Ganin2016DomainAdversarial, Tzeng2017Adversarial}. As depicted in Figure~\ref{fig:adversarial_da}, the feature extractor learns to maximize domain confusion while the domain discriminator attempts to distinguish source from target samples, creating a minimax game that drives domain-invariant representation learning \cite{Goodfellow2014GAN, Ganin2015UnsupervisedDA}. While powerful and end-to-end trainable, adversarial methods can suffer from training instability, mode collapse, and sensitivity to hyperparameters \cite{Arjovsky2017Wasserstein, Gulrajani2017ImprovedWGAN}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/adversarial_domain_adaptation.pdf}
    \caption{Adversarial domain adaptation methods. A gradient reversal layer enables simultaneous task learning and domain confusion through adversarial training dynamics.}
    \label{fig:adversarial_da}
\end{figure}

\subparagraph{Autoencoder-Based Domain Adaptation.}
Autoencoder-based methods learn shared latent representations by training encoders and decoders to reconstruct both source and target domain inputs from a common bottleneck representation \cite{Ghifary2016DeepReconstruction, Bousmalis2016DSN}. As shown in Figure~\ref{fig:autoencoder_da}, the reconstruction constraint encourages the latent space to capture domain-invariant structural information while discarding domain-specific noise and artifacts \cite{Liu2016CoupledGAN, Tzeng2015SimultaneousDRC}. Task predictors operate directly on these shared representations, enabling transfer without explicit domain labels. However, reconstruction-based objectives may preserve information irrelevant to the task or lose discriminative features critical for downstream prediction \cite{Pinheiro2018UnsupervisedDA, Hoffman2018CyCADA}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Figures/autoencoder_domain_adaptation.pdf}
    \caption{Autoencoder-based domain adaptation architectures. Shared encoders and domain-specific decoders with reconstruction losses learn domain-invariant latent representations.}
    \label{fig:autoencoder_da}
\end{figure}

\subparagraph{Model-Based Domain Adaptation.}
Model-based approaches leverage pre-trained models from source domains and adapt them to target domains through transfer learning, fine-tuning, or feature extraction \cite{Yosinski2014Transferable, Zhuang2020Comprehensive}. As illustrated in Figure~\ref{fig:model_based_da}, common strategies include full fine-tuning of all network layers on limited target data, selective fine-tuning of task-specific layers while freezing feature extractors, and using pre-trained features as fixed representations with new task heads \cite{Kumar2022FineTuning, Guo2020SpotTune}. These methods are particularly effective when source and target domains share structural similarities and when target data is scarce, but risk negative transfer if domains are too dissimilar or if inappropriate layers are adapted \cite{Tan2018SurveyDL, Wang2019CharacterizingTransferability}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Figures/model_based_domain_adaptation.pdf}
    \caption{Model-based domain adaptation approaches. Pre-trained models can be adapted through fine-tuning, transfer learning, or feature extraction when target domain data is limited.}
    \label{fig:model_based_da}
\end{figure}

\subparagraph{Hybrid Domain Adaptation.}
Hybrid approaches combine multiple adaptation strategies to leverage their complementary strengths and mitigate individual limitations \cite{Peng2019MomentMatching, Saito2018MaxClassifier}. As shown in Figure~\ref{fig:hybrid_da}, a typical hybrid framework might integrate data augmentation for source domain enrichment, feature alignment through MMD or CORAL, adversarial training for domain confusion, transfer learning for initialization, and instance weighting for sample importance \cite{Long2018ConditionalDA, Xu2019LargerNorm}. By operating at data, feature, and model levels simultaneously, hybrid methods achieve robust performance across diverse domain shifts, though at the cost of increased implementation complexity and computational overhead \cite{Saito2019SemiSupervisedDA, Shu2018DirtT}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Figures/hybrid_domain_adaptation.pdf}
    \caption{Hybrid domain adaptation frameworks. Multiple adaptation strategies can be combined at different levels (data, feature, model) to achieve more robust cross-domain performance.}
    \label{fig:hybrid_da}
\end{figure}

The choice among these domain adaptation strategies depends on the specific characteristics of the sim-to-real gap in magnetic domain imaging: the availability of target domain data, the nature and magnitude of distribution shifts, computational constraints, and the interpretability requirements of the application \cite{Wilson2020Survey, Wang2018DeepVisualDA}. For magnetic nanodot systems, where experimental data is expensive and limited, while simulation data is abundant but systematically biased, hybrid approaches combining model-based transfer learning with feature-level alignment and data-level augmentation offer a promising path toward robust sim-to-real generalization \cite{DeepSPIN2023, Hilgers2025}.

\paragraph{Summary and research gaps.}
Despite significant progress in domain adaptation methods, several challenges remain specifically relevant to magnetic domain imaging. First, most domain adaptation techniques assume smooth, continuous distribution shifts, whereas the simulation-experiment gap in magnetic imaging involves discrete instrumental modalities (MFM vs. SP-STM vs. Lorentz TEM) with fundamentally different physical measurement principles \cite{Raabe2020ImagingLimits}. Second, current methods often require access to unlabeled target domain data during training, which may not always be available for expensive experimental techniques \cite{Schlenhoff2022SPSTM}. Third, domain adaptation approaches typically focus on classification or regression tasks, with limited exploration of generative modeling for magnetic textures where both forward and inverse problems must be addressed simultaneously \cite{Park2024}. Finally, the interaction between domain adaptation and physics-informed constraints remains underexplored: how to preserve physical consistency (energy minimization, micromagnetic equilibrium) while adapting across domains is an open research question \cite{Karniadakis2021PINNs, DeepSPIN2023}. Table~\ref{tab:domain_adaptation_comparison} provides a comprehensive comparison of the key domain adaptation strategies discussed above, highlighting their respective strengths and limitations in the context of magnetic domain imaging.

\subsection*{Summary table: Domain adaptation strategies}
...
\hline
\textbf{Strategy} & \textbf{Key strengths} & \textbf{Main limitations} \\
\hline
Data-Based
& Simple implementation; no architecture changes; computationally efficient
& Requires accurate modeling of target distribution; manual tuning intensive \\
\hline
Instance-Based
& No target labels required; flexible; test-time adaptation possible
& Sensitive to large distribution shifts; requires confidence calibration \\
\hline
Feature-Based
& Theoretically grounded; aligns intermediate representations; reduces domain gap
& May discard task-relevant features; careful hyperparameter tuning needed \\
\hline
Adversarial
& Powerful alignment; end-to-end trainable; domain-invariant features
& Training instability; mode collapse; hyperparameter sensitivity \\
\hline
Autoencoder-Based
& Learns shared representations; unsupervised adaptation; preserves structure
& May lose discriminative features; reconstruction quality critical \\
\hline
Model-Based
& Leverages source knowledge; effective with limited target data; well-established
& Risk of negative transfer; requires careful layer selection \\
\hline
Hybrid
& Combines multiple strengths; robust across diverse shifts; state-of-the-art performance
& Implementation complexity; increased computational cost \\
\hline
\end{tabular}
\caption{Comparison of domain adaptation strategies for bridging the sim-to-real gap in magnetic domain imaging.}
\label{tab:domain_adaptation_comparison}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Physics-informed constraints and energy penalties for interpretable inverse modeling.} Illustration of energy-based penalties and physical consistency constraints that can be incorporated into neural network training to enforce interpretability and physical traceability. These methods include explicit Hamiltonian energy terms, exchange energy penalties, and consistency losses that ensure predicted magnetic parameters generate physically plausible equilibrium states when passed through forward micromagnetic models.}
    \label{fig:physics_penalties}
\end{figure}
...
\textbf{Domain structures present:} vortex states, stripe domains, helical textures, mixed ferromagnetic-antiferromagnetic configurations.
...
\subsubsection{Simulation Details and Parameter Ranges}
...
To this end, Bayesian hyperparameter optimization is employed (Figure~\ref{fig:bayesian_optimization}) to systematically identify the optimal configuration
\begin{equation}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Bayesian hyperparameter optimization workflow.} A Gaussian process surrogate models the validation objective, guiding acquisition of optimal hyperparameters via uncertainty-aware exploration.}
...
\subsection{Domain Adaptation for Sim-to-Real Generalization}
...
\subsubsection{Domain Adaptation Approach}
...
\subsubsection{Integration with Cycle-Consistent Framework}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Figures/domain_adapted_cycle.pdf}
...
The full interpretability pipeline (Figure~\ref{fig:interpretability_strategy}) proceeds as follows:
\begin{enumerate}
...
\subsection{Datasets and Experimental Configuration}\label{sec:Data}

The experimental evaluation employs the synthetic training datasets described in Section~\ref{sec:training_datasets}: the KDM DATABASE SPINERS dataset (164,212 images focusing on temperature and KDM parameter variations) and the MATERIAL SPINNERS DATA dataset (54,404 images exploring exchange interaction parameter regimes). Both datasets provide $39 \times 39$ pixel grayscale magnetic domain images with corresponding Hamiltonian parameter labels, covering complementary regions of the physical parameter space. Detailed descriptions of the Hamiltonian formulation, simulation procedures, parameter ranges, and dataset structure are provided in Section~\ref{sec:databases}. Representative examples of the magnetic domain configurations from both datasets are shown in Figures~\ref{fig:kdm_dataset} and~\ref{fig:spinner_dataset}.
...
A central research direction will focus on bridging the sim-to-real gap through systematic domain adaptation strategies. As discussed in Section~\ref{sec:sim2real}, the distribution shift between idealized simulations and experimental observations represents a fundamental challenge for deploying learned models in practice. Future work will empirically evaluate and compare multiple domain adaptation approaches—including data-level augmentation, feature-level statistical alignment, and hybrid strategies—to determine the most effective combination for magnetic domain imaging. The experimental datasets described in the methodology (Leeds skyrmions, Co/Pd MFM, KIT 4D-STEM) will serve as testbeds for validating domain adaptation performance. Particular emphasis will be placed on ensuring that domain-adapted models preserve physical consistency and do not introduce spurious magnetic features while adapting to experimental imaging conditions. The interplay between domain adaptation losses and physics-informed constraints will be carefully investigated to maintain both realism and physical plausibility.
